...
namespace Microsoft.Docs.Build
{
    ...
    internal static class Errors
    {
        public static class System
        {
            ...
            {
                return new Error(ErrorLevel.Error, "git-clone-failed", $"Failure to clone the repository `{url}#{branch}`. This could be caused by an incorrect repository URL, please verify the URL on the Docs Portal (https://ops.microsoft.com). This could also be caused by not having the proper permission the repository,  please confirm that the GitHub group/team that triggered the build has access to the repository.");
            }
        ...
        public static class Redirection
        {
            ...
            {
                return new Error(ErrorLevel.Suggestion, "redirect-url-invalid", $"Can't redirect document ID for redirected file '{from}' because redirect URL '{source}' is invalid or is in a different docset. Specify a redirect_url in the same docset, or set redirect_document_id to false in .openpublishing.redirection.json.", source);
            }
        ...
        public static class DependencyRepository
        {
            ...
            {
                return new Error(ErrorLevel.Error, "restore-dependent-repository-failed", $"Failed to restore dependent repository `{url}#{branch}`. This could be caused by an incorrect repository URL, please verify the URL on the Docs Portal (https://ops.microsoft.com). If it is not the case, please open a ticket in https://SiteHelp and include URL of the build report.");
            }
